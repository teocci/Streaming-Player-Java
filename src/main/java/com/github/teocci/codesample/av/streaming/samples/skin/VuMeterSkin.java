package com.github.teocci.codesample.av.streaming.samples.skin;

import com.github.teocci.codesample.av.streaming.samples.Section;
import com.github.teocci.codesample.av.streaming.samples.VuMeter;
...
public class VuMeterSkin extends SkinBase<VuMeter> implements Skin<VuMeter>
{
    ...
    public static final long PEAK_TIMEOUT = 500_000_000l;
    ...
    private void init()
    {
        if (Double.compare(getSkinnable().getPrefWidth(), 0.0) <= 0 || Double.compare(getSkinnable().getPrefHeight(), 0.0) <= 0 ||
            ...
            if (Orientation.HORIZONTAL == getSkinnable().getOrientation()) {
                getSkinnable().setPrefSize(
                        getSkinnable().getNoOfLeds() * PREFERRED_WIDTH + (getSkinnable().getNoOfLeds() - 1) * getSkinnable().getLedSpacing(),
                        PREFERRED_HEIGHT
                );
            } else {
                getSkinnable().setPrefSize(
                        PREFERRED_WIDTH,
                        getSkinnable().getNoOfLeds() * PREFERRED_HEIGHT + (getSkinnable().getNoOfLeds() - 1) * getSkinnable().getLedSpacing()
                );
            }
        }
...
//        hBox.setSpacing(getSkinnable().getLedSpacing());
            ...
            if (Orientation.HORIZONTAL == getSkinnable().getOrientation()) {
                ...
                if (getSkinnable().isPeakValueVisible()) {
                    if (currentLedPeakIndex > peakLedIndex) {
                        animatePeakLed(currentLedPeakIndex);
                    }
                }
            } else {
                // VERTICAL
    ...
    private void animatePeakLed(int currentLedPeakIndex)
    {
        timer.stop();
        leds.get(peakLedIndex).getStyleClass().remove("led-peak");
    ...
    private void initLedStyle(Region led, int i)
    {
