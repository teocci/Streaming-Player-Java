...
public class SimpleAudioConversion
{
     ...
     * @throws NullPointerException           if {@code bytes}, {@code samples} or {@code fmt} is {@code null}
     * @throws ArrayIndexOutOfBoundsException if {@code (bytes.length < blen)}
     ...
     * @throws NullPointerException           if {@code samples}, {@code bytes} or {@code fmt} is {@code null}
     * @throws ArrayIndexOutOfBoundsException if {@code(samples.length < slen)}
    ...
    {
        if (isBigEndian) {
            return (((bytes[i] & 0xffL) << 8L) | (bytes[i + 1] & 0xffL));
        } else {
            return ((bytes[i] & 0xffL) | ((bytes[i + 1] & 0xffL) << 8L));
        }
    }
    ...
    {
        if (isBigEndian) {
            return (((bytes[i] & 0xffL) << 16L) | ((bytes[i + 1] & 0xffL) << 8L) | (bytes[i + 2] & 0xffL));
        } else {
            return ((bytes[i] & 0xffL) | ((bytes[i + 1] & 0xffL) << 8L) | ((bytes[i + 2] & 0xffL) << 16L));
        }
    }
    ...
    private static long unpackAnyBit(byte[] bytes, int i, boolean isBigEndian, int bytesPerSample)
    {
        ...
        if (isBigEndian) {
            for (int b = 0; b < bytesPerSample; b++) {
                temp |= (bytes[i + b] & 0xffL) << (8L * (bytesPerSample - b - 1L));
            }
        } else {
            for (int b = 0; b < bytesPerSample; b++) {
                                   ...
                                   int bytesPerSample)
    {
        if (isBigEndian) {
            for (int b = 0; b < bytesPerSample; b++) {
                bytes[i + b] = (byte) ((temp >>> (8L * (bytesPerSample - b - 1L))) & 0xffL);
            }
        } else {
            for (int b = 0; b < bytesPerSample; b++) {
